...
- **Sem WebGL2**, o robô é desenhado em Canvas 2D pelo `web/twin2d.js`. Cada
  ...
  pintor fazem o resto. São 9 kB (`web/silhueta.json`) contra 0,55 MB de malha.
- **Sem Pointer Events**, as teclas de jog e a órbita da câmera usam Touch
  Events, com o mouse como terceira opção.

A silhueta é aproximada por construção: fecho convexo não tem buraco, então os
recortes da torre e do punho fecham. A pose, o tamanho e a direção da
ferramenta continuam exatos, porque saem das mesmas transformações — inclusive
o acoplamento J2-J3.
